...
Configuration is now read into a list of dictionaries for improved stability and clarity.
"""
...
class CameraHandler:
    ...
    def setup_camera(self, stack):
        ...
        try:
            ...
            # Log device info
            # device_id = self.device_info.getMxId()
    ...
    def process_frames(self, clock):
        ...
        while self.running and self.queue:
            try:
                ...
                # Check if this is a keyframe (for logging only)
    ...
    def stop(self):
        ...
        if self.thread:
            # Note: The ExitStack/dai.Device will close the underlying resource
            # which will cause the self.queue.get() to raise an exception,
            # cleanly terminating the thread. A small timeout is included.
...
class OAKMultiCameraH264Node(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            streams_config = config['/**']['ros__parameters']['streams']
        except Exception as e:
            self.get_logger().error(f"Failed to load or parse streams.yaml: {e}")
            raise

        # --- MODIFIED CONFIGURATION LOADING FOR ROBUSTNESS (List of Dictionaries) ---
        camera_names = streams_config['names']
        camera_ips = streams_config['camera_ips']
        widths = streams_config['widths']
        heights = streams_config['heights']
        fps_list = streams_config['fps']
        bitrates = streams_config['bitrates']

        self.camera_configs = []
        if not (len(camera_names) == len(camera_ips) == len(widths) == len(heights) == len(fps_list) == len(bitrates)):
            self.get_logger().fatal("Configuration lists in streams.yaml are mismatched in length. Cannot proceed.")
            raise ValueError("Mismatched list lengths in configuration.")
        ...
        # --- END MODIFIED CONFIGURATION LOADING ---
        ...
        # Dictionary to store camera handlers, keyed by name
        self.cameras = {} 
        print(self.camera_configs)
        # Iterate over the robust list of dictionaries
        for camera_config in self.camera_configs:
            ...
            if camera.setup_camera(self.stack):
                self.cameras[camera_name] = camera # Store in dictionary
        ...
        total_configs = len(self.camera_configs)
        initialized_count = len(self.cameras)
        self.get_logger().info(f'Successfully initialized {initialized_count}/{total_configs} cameras')

        # Start frame processing for all cameras
        for camera_handler in self.cameras.values():
            camera_handler.start_processing(self.get_clock())
